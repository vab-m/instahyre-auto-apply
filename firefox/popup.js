// Popup script for Instahyre Auto Apply Extension (Firefox Version)

document.addEventListener('DOMContentLoaded', async () => {
    // Get current tab - Firefox uses browser.tabs instead of chrome.tabs
    ...
    // Load saved settings
    const settings = await browser.storage.local.get(['delay', 'skipApplied', 'autoScroll']);
    ...
    delayInput.addEventListener('change', () => {
        browser.storage.local.set({ delay: parseInt(delayInput.value) });
    });
    skipAppliedCheckbox.addEventListener('change', () => {
        browser.storage.local.set({ skipApplied: skipAppliedCheckbox.checked });
    });
    autoScrollCheckbox.addEventListener('change', () => {
        browser.storage.local.set({ autoScroll: autoScrollCheckbox.checked });
    });
